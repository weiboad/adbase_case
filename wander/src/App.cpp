...
void App::run() {
    _bs = new BinarySearch();
    _bs->loadZipCode(_configure->ip2zipcode, "\t");
}
...
void App::reload() {
    _bs->reLoadZipCode(_configure->ip2zipcode, "\t");
}
...
void App::loadConfig(adbase::IniConfig& config) {
    _configure->ip2zipcode = config.getOption("zipcode", "ip2zipcode");
	LOAD_TIMER_CONFIG(Test);
}
